...
def show():
    ...
    if uploaded_file is not None:
        try:
            ...
            num_cols = st.slider("Select column range to view: ",min_value = 1,max_value = df.shape[1],value = (1,min(5,df.shape[1])))
            num_rows = st.slider("Select row range to view: ",min_value=1,max_value = df.shape[0],value = (1,min(5,df.shape[0])))
